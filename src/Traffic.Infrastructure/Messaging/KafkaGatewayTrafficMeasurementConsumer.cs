...
public sealed class KafkaGatewayTrafficMeasurementConsumer :
    ...
    IDisposable
{
    ...
    private readonly ILogger<KafkaGatewayTrafficMeasurementConsumer> _logger;
    private readonly string _topicName;

    public KafkaGatewayTrafficMeasurementConsumer(
        KafkaSettings settings,
        ILogger<KafkaGatewayTrafficMeasurementConsumer> logger)
    {
        ...
        _consumer = new ConsumerBuilder<string, string>(new ConsumerConfig
            {
                ...
                GroupId = settings.ConsumerGroups.GatewayMeasurements.Trim(),
                ClientId = "traffic-gateway-api-measurements",
    ...
    private static void ValidateSettings(KafkaSettings settings)
    {
        ...
        if (settings.ConsumerGroups is null ||
            string.IsNullOrWhiteSpace(settings.ConsumerGroups.GatewayMeasurements))
        {
            throw new InvalidOperationException(
                "Kafka ConsumerGroups GatewayMeasurements must be configured.");
        }
    }
}
